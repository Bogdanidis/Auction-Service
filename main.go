...
var auctionServicePort = "localhost:8080"
var auctionEvaluationServicePort = "localhost:8081"
...
func main() {
	...
	router.PATCH("/auctions/:id/assets/:assetid", database.PatchAuctionAsset)

	router.DELETE("/database", database.DeleteDatabase)
	router.DELETE("/auctions/:id", database.DeleteAuction)

	router.Run(auctionServicePort)
